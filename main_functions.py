...
def PredictGoal(df):
    ...
    if predicted_outcome.argmax().item() == 1:
        return "The predicted outcome is likely a no goal."
    else:
        return "The predicted outcome is likely a goal."
...
def call_IkomiaAPI():
  ...
  with open(r'UserImage.jpg', 'rb') as image_file:
      ...
      base64_string = base64_bytes.decode()
  ######################## Execute deployment  ##############################
  url = "https://zk3ppi1ux4.execute-api.eu-central-1.amazonaws.com/api/run"
  payload = json.dumps({
    "inputs": [
      {"image": f"{base64_string}"}
    ],
...
def fetch_workflow_results(response, JWT):
    ...
    while timeout > 0:
        ...
        # uuid = response.text
        uuid_json = response.json()
        Endpoint_URL = 'https://zk3ppi1ux4.execute-api.eu-central-1.amazonaws.com'
        url = f"{Endpoint_URL}/api/results/{uuid_json}"
        payload = {}
        headers = {
            'Accept': 'application/json',
